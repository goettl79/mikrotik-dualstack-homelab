...
# 1. setup.rsc (local unencrypted private config)
# 2. setup.enc.rsc (SOPS-encrypted config, decrypted on-the-fly)
TMP_DECRYPTED=""
if [[ -f "${SCRIPT_DIR}/setup.rsc" ]]; then
    SCRIPT_FILE="setup.rsc"
    CONFIG_PATH="${SCRIPT_DIR}/${SCRIPT_FILE}"
elif [[ -f "${SCRIPT_DIR}/setup.enc.rsc" ]] && command -v sops >/dev/null 2>&1; then
    echo "[*] Decrypting setup.enc.rsc via SOPS..."
    TMP_DECRYPTED="$(mktemp --suffix=.rsc)"
    trap 'rm -f "${TMP_DECRYPTED}"' EXIT
    sops -d --input-type binary --output-type binary "${SCRIPT_DIR}/setup.enc.rsc" > "${TMP_DECRYPTED}"
    SCRIPT_FILE="setup.rsc"
    CONFIG_PATH="${TMP_DECRYPTED}"
fi
